...
public class sAttackLogic : MonoBehaviour
{
        ...
        for (int i = 0; i < aTiles.Length; i++)
        {
            if (aTiles[i].transform.parent.GetComponent<sTileLogic>().walkable == true)
            {
                ...
                if (distance <= 3.5 && distance > 0.5)
                {
                    ...
                    tileChild.GetComponent<sVisualizerLogic>().AssignInteraction("Attack");
                    tileChild.GetComponent<SpriteRenderer>().color = Color.red;
                }
            }
        }
    }
